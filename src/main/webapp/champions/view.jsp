...
<table border=0>
	<tr>
		...
		<td width=890 valign=top align=center>
			<table width="700" border="0" cellpadding="0" cellspacing="0">
				<tr height=30></tr>
				<tr>
					<td colspan=3 align=center style="color:blue; font-size: 30px; font-weight: bold;">★<%=bbs_title%>★</td>
				</tr>
				<tr height=30></tr>
				<tr>
 ...
 %>
			<table width="700" border="0" cellpadding="0" cellspacing="0">
				...
				<tr>
					...
					<td><%=rs_comment.getString("tb_comment")%> <%
 if (session_level != null && (session_level.equals("10") || mem_id.equals(session_id))) {
 %> <img src="img/comment_del.gif"
						onclick="comment_delete(<%=mem_uid%>)"> <%
 }
 %></td>
					<td><%=rs_comment.getString("tb_date")%></td>
				</tr>
				<tr>
					<td colspan=3 height=1 width=100% bgcolor=#97b1ec></td>
				</tr>
				<%
				}
				%>
			...
			<form action="comment_insert.jsp" method="post"
				...
				<table width="700" border="0" cellpadding="0" cellspacing="0">
					<tr height=30>
						<td width=610><input id="view_comment" name="view_comment"
							style="width: 99%"></td>
 ...
 %> <input type="button"
							onclick="javascript:alert('로그인 후 사용 가능합니다..')" value="댓글등록">
							<%
							}
							%>
						</td>
					...
					<tr>
						<td colspan=2 height=1 width=100% bgcolor=#97b1ec></td>
					</tr>
				</table>
			</form> <script>
			function send(){
			...
			<table width="700" border="0" cellpadding="0" cellspacing="0">
				<tr>
					<td><a
						href="list.jsp?code=<%=code%>&field=<%=field%>&search=<%=search%>&page_now=<%=page_now%>">[목록]</a></td>
					<td align=right>
						<%
						if (session_level != null && (session_level.equals("10") || session_id.equals(id))) {
						%>
						<a
						href="modify.jsp?code=<%=code%>&uid=<%=uid%>&page_now=<%=page_now%>">[수정]</a>
						<a onclick="delete_go()">[삭제]</a> <%
					    } else {
 %> <span
						onclick="alert('작성자만 수정하실 수 있습니다.')">[수정]</span> <span
						onclick="alert('작성자만 삭제하실 수 있습니다.')">[삭제]</span> <%
 }
 %>
					</td>
				</tr>
		...
		<td width=260 valign=top style="padding-left: 20px"><br> <%@ include
				file="/member/login.jsp"%></td>
		<td></td>
	</tr>
</table>
